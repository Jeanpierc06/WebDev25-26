...
Notes:
1.1 guve each info card div a class of "card"
1) Images
...
2) Create an appropriate random price to display for each pizza( say 15 - 35) - HTML element needed
3) Enter quantity is a text input, no need to give it an ID
4) Buy is a button input, no need to give it an event listener
...
function init(){
  ...
  for( let i = 1' i<=5; i += 1 ) 
   let price = Math.pow(Math.random()* (35+15+1) +15)
  --- HTML
